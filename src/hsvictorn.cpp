...
uint8_t select8bitfrom16bit(uint16_t var, uint16_t bits)
{
    ...
    {
        if ((bits >> bit) & 1)
        { // if this is one of the wanted bits
            if ((var >> bit) & 1)
                newvar = newvar | (1 << pos); // Copy vanted bit to newvar
            pos++;
        }
    }
    return (newvar);
  ...
  */
    ...
    { //Wait data to become available
        ...
        if (maxdelay < 1)
        {
            ...
            if (sensorStatus == 1)
                sensorStatus = 3; // Both reads faild
            else
                sensorStatus = 2; // only inverter read fail
            break;
        }
    };
    delay(50);
